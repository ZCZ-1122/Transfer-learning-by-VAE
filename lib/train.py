...
def train(model, optimizer, loss_fn, dataloader, metrics, params):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, train_batch in enumerate(dataloader):
            ...
            image_batch, label_batch = train_batch['image'], train_batch['label']
            image_batch  = image_batch.to(device=params.device, dtype=params.dtype)
            label_batch  = label_batch.to(device=params.device, dtype=torch.long)
...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, loss_fn, metrics, params,
    ...
    for epoch in range(params.num_epochs):
        ...
        writer.add_scalar("Dice/train", tr_epoch_hist['dice'], epoch)
        writer.add_scalar("Dice/val", val_epoch_hist['dice'], epoch)
